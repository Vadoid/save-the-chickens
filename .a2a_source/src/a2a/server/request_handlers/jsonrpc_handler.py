...
class JSONRPCHandler:
    ...
    async def on_message_send(
        ...
        try:
            print(f'DEBUG_HANDLER: request type {type(request)}')
            print(f'DEBUG_HANDLER: request vars {vars(request) if hasattr(request, '__dict__') else 'no dict'}')
            task_or_message = await self.request_handler.on_message_send(
                request.params if hasattr(request, 'params') else request, context
            )
            print(f'DEBUG_HANDLER: result type {type(task_or_message)}')
            print(f'DEBUG_HANDLER: isinstance Message? {isinstance(task_or_message, Message)}')
            if isinstance(task_or_message, SendMessageResponse):
                return task_or_message
            return prepare_response_object(
                getattr(request, 'id', 0),
                task_or_message,
                (Task, Message),
                SendMessageSuccessResponse,
                SendMessageResponse,
            ...
            return SendMessageResponse(
                root=JSONRPCErrorResponse(
                    id=getattr(request, 'id', 0), error=e.error if e.error else InternalError()
                )
            )
    ...
    async def on_cancel_task(
        ...
        return CancelTaskResponse(
            root=JSONRPCErrorResponse(id=getattr(request, 'id', 0), error=TaskNotFoundError())
        )
        ...
        try:
            ...
            return DeleteTaskPushNotificationConfigResponse(
                root=DeleteTaskPushNotificationConfigSuccessResponse(
                    id=getattr(request, 'id', 0), result=None
                )
            )
        except ServerError as e:
            return DeleteTaskPushNotificationConfigResponse(
    ...
    async def get_authenticated_extended_card(
        ...
        return GetAuthenticatedExtendedCardResponse(
            root=GetAuthenticatedExtendedCardSuccessResponse(
                id=getattr(request, 'id', 0), result=card_to_serve
            )
        )
